...
MUTATIONS = [
    ...
    ("the Chat page stays 900 tall however long the word list", "Options.lua",
     ...
     ""),
    ("the search box does not filter our list", "TrainerUI.lua",
    ...
    ("Blizzard's search preview covers our list", "TrainerUI.lua",
     "function(self) if Ours() then self:HidePreviewResultSearch() end end",
     "function(self) end"),
    ("Enter's full search leaves the dimmed tab to be restored as selected", "TrainerUI.lua",
     "if Ours() then UndimTheirs(false) UI.Refresh() end",
     "if Ours() then UI.Refresh() end"),
    ("the search filter is case-sensitive", "TrainerUI.lua",
     "e.name:lower():find(q, 1, true)",
     "e.name:find(q, 1, true)"),
    ("the tab's icon is .icon only, so a skin that spares .Icon blanks it", "TrainerUI.lua",
